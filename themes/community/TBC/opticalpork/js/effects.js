...
Element.collectTextNodesIgnoreClass = function (element, className) {
	return $A($(element).childNodes).collect(
		function (node) {
			return (node.nodeType == 3 ? node.nodeValue : ((node.hasChildNodes() && !Element.hasClassName(node,className)) ? Element.collectTextNodes(node) : ''));
		}
	).flatten().join('');
}
...
Element.setStyle = function (element, style) {
	element = $(element);
	for (k in style) {
		element.style[k.camelize()] = style[k];
	}
}
...
Element.setOpacity = function (element, value) {
	...
	if (value == 1) {
		Element.setStyle(
			element,
			{ opacity:
				(/Gecko/.test(navigator.userAgent) && !/Konqueror|Safari|KHTML/.test(navigator.userAgent)) ? 0.999999 : null }
		);
...
var Effect = {
	...
	toggle: function (element, effect) {
		...
		Effect[Element.visible(element) ? Effect.PAIRS[effect][1] : Effect.PAIRS[effect][0]](element, options);
	}
};
...
Object.extend(
	Object.extend(Effect.ScopedQueue.prototype, Enumerable),
	{
		initialize: function () {
			this.effects  = [];
			this.interval = null;
	...
	this.effects._each(iterator);
		...
		remove: function (effect) {
			this.effects = this.effects.reject(
				function (e) {
					return e == effect }
			);
...
Effect.Base.prototype = {
	...
	start: function (options) {
		...
		if (!this.options.sync) {
			Effect.Queues.get(
				typeof this.options.queue == 'string' ? 'global' : this.options.queue.scope
			).add(this);
		}
	},
...
Object.extend(
	Object.extend(Effect.Parallel.prototype, Effect.Base.prototype),
	{
...
Object.extend(
	Object.extend(Effect.Opacity.prototype, Effect.Base.prototype),
	{
...
Object.extend(
	Object.extend(Effect.Move.prototype, Effect.Base.prototype),
	{
		initialize: function(element) {
			this.element = $(element);
			var options  = Object.extend(
...
Object.extend(
	Object.extend(Effect.Scale.prototype, Effect.Base.prototype),
	{
...
Object.extend(
	Object.extend(Effect.ScrollTo.prototype, Effect.Base.prototype),
	{
		initialize: function(element) {
			this.element = $(element);
			this.start(arguments[1] || {});
