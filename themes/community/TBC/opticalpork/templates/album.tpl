...
{else}
  ...
  {/if}
<div class="bigone">
{* g->block type="imageblock.ImageBlock" blocks="recentImage" show=fullSize useDefaults=0 *}
</div>
  ...
  {foreach from=$theme.children item=child}
    {if !$child.canContainChildren}
      ...
      <div class="gallery-thumb">
        {* do nifty hidden exif block *}
        <div id="exif" style="display: none">
          &nbsp;
        </div>

	{if isset($theme.resizeId[$child.id])}
	<a href="{g->url arg1="view=core.DownloadItem" arg2="itemId=`$theme.resizeId[$child.id]`"}" capt='<h4>{$child.title|markup|wordwrap:10:"<span style=\"font-size: 0px\"> </span>":true}</h4><br><a href="{g->url arg1="view=core.ShowItem" arg2="itemId=`$child.id`"}">View Photo Page</a><br>{g->block type=exif.ExifInfo item=`$child`}' rel="lightbox[photos]">
	{else}
	<a href="{g->url arg1="view=core.DownloadItem" arg2="itemId=`$child.thumbnail.id`"}" capt='<h4>{$child.title|markup|wordwrap:10:"<span style=\"font-size: 0px\"> </span>":true}</h4><br><a href="{g->url arg1="view=core.ShowItem" arg2="itemId=`$child.id`"}">View Photo Page</a><br>{g->block type=exif.ExifInfo item=`$child`}' rel="lightbox[photos]">
	{/if}
          {if isset($child.thumbnail)}
            {g->image item=$child image=$child.thumbnail}
          {else}
