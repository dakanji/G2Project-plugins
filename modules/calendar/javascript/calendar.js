...
function initFormatting() {
	if (browser != 'Internet Explorer') {
		window.onresize = function () {
			showLoading(); format(); hideLoading(); };
		format();
	}
}
...
function initEvent(event) {
	...
	event.onmouseover = function (ev) {
		mouseOverEvent(ev, this) };
	event.onmouseout  = function (ev) {
		mouseOutEvent(ev, this) };
	event.onmousedown = function (ev) {
		mouseDownEvent(ev, this) };

	var text         = getEventText(event);
	text.onmouseover = function (ev) {
		mouseOverEvent(ev, this.parentNode.parentNode); };
	text.onmouseout  = function (ev) {
		mouseOutEvent(ev, this.parentNode.parentNode); };
	text.onmousedown = function (ev) {
		mouseDownEvent(ev, this.parentNode.parentNode); };

	// prevent IE text selection!!!
	event.ondrag        = function () {
		return false; };
	event.onselectstart = function () {
		return false; };
	//}
}
...
function mouseDownEvent(e, event) {
	try {
		...
		document.onmousemove = function (ev) {
			mouseMoveEvent(ev, savedEvent) };
		document.onmouseup   = function (ev) {
			mouseUpEvent(ev, savedEvent) };
		...
		if (savedPart != null) {
			if (movedDistance(savedX, savedY, mouseX, mouseY) > 5) {
				if (savedPart == 'text') {
					...
					Drag.init(dragEvent);
					dragEvent.onDragEnd = function (x, y) {
						dropEvent(event, dragEvent, x, y); };
					...
					dragResize.onDragEnd = function (x, y) {
						dropResize(event, dragResize, part, x, y); };
...
function dropEvent(event, dragEvent, x, y) {
	...
	if (singleDay == null) {
		...
		req.onload = function (data, obj) {
			moveAjaxCallback(event, singleDay, dragEvent, data, obj); };
		req.onfail = function (status) {
			moveAjaxCallback(event, singleDay, dragEvent, status, null) };
...
function dropResize(event, dragResize, part, x, y) {
	...
	if ((part == 'start' && date > eventEndDate) || (part == 'end' && date < eventStartDate)) {
		...
		req.onload = function (data, obj) {
			resizeAjaxCallback(event, dragResize, data, obj); };
		req.onfail = function (status) {
			resizeAjaxCallback(event, dragResize, status, null) };
		req.getURL(baseURL + 'update/');
	}
}
...
function findSingleDayAtPos(x, y) {
	var singleDays = calendar.getElementsByTagName('td');
...
function initNewEventButtons() {
	...
	var calendar  = document.getElementById('calendar');
	var newEvents = getElementsByClass(calendar, 'new-event');

	for (var ct1 = 0; ct1 < newEvents.length; ct1++) {
		var element     = newEvents[ct1];
		element.onclick = function () {
			addNewEvent(getDaySingleEvents(this)); };

		var date        = element.title;
...
function addNewEvent(startDate, endDate) {
	log('adding new event');

	try {
		var element  = document.getElementById(startDate + '-' + endDate);
...
function getDaySingleEvents(newEventButton) {
	log('getting day single events for: ' + newEventButton);
...
function editEvent(event) {
	try {
		...
		del.onclick = function () {
			ajaxDeleteEvent(this.parentNode.parentNode); };
		...
		input.onkeydown = function (e) {
			editKeyHandler(e, getInputEvent(this)) };

		workingEvent         = event;
		document.onmousedown = function (e) {
			documentClick(e, workingEvent) };

		//input.onblur = function() { inputBlur(this); };
		form.onsubmit = function () {
			editComplete(this);  return false; };
		input.select();
		input.focus();
	} catch (e) {
		logError(e);
...
function editComplete(form) {
	...
	try {
		...
		if (input.value != eventText.innerHTML || hasClass(event, UPDATE_FAILED_CLASS)) {
			...
			req.onload = function (data, obj) {
				editAjaxCallback(event, data, obj); };
			req.onfail = function (status) {
				editAjaxCallback(event, status, null) };
			req.getURL(baseURL + 'update/');
		} else {
...
var oldHandlingFunction = (window.onload != null) ? window.onload : function () {};
