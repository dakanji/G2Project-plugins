...
<div class="gbBlock gcBackground1">
  <h2 class="gbTitle"> {g->text text="Missing Tags"} </h2>
</div>
...
<br>
    ...
    {if count($baditems) > 0}
	{g->text text="The following items are missing tag entries:"}
	...
	<ol>
		{foreach from=$baditems item=baditempair}
			...
			&nbsp;
			<a href="{g->url arg1="view=tags.TagsItemEdit"
					 arg2="itemId=`$baditem->id`"}">

	  	        {g->text text="Edit-Tags"}
	    		</a>
    ...
    {else}
	{g->text text="There are no items that have missing tag entries."}
    {/if}	
