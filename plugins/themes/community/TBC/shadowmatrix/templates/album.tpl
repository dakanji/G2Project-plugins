            ...
            <div id="gsContent" class="gcBorder1">
                <div class="gbBlock gcBackground1">
                    <table style="width: 100%">
                        <tr>
                            <td>
                                {if !empty($theme.item.title)}<h2>{$theme.item.title|markup} </h2>
                                {/if}
                ...
                <div class="gbBlock giDescription gbEmptyAlbum">
                    <h3 class="emptyAlbum">{g->text text="This album is empty."}
                        ...
                        <br/>
                        <a href="{g->url arg1="view=core.ItemAdmin" arg2="subView=core.ItemAdd" arg3="itemId=`$theme.item.id`"}"> { g->text text="Add a photo!"}  </a>{/if}
                    </h3>
                </div>
                {else}
                {assign var="childrenInColumnCount" value=0}
                <div class="gbBlock">
                    <table id="gsThumbMatrix" cellpadding="0" cellspacing="0">
                        <tr valign="top">
                            {foreach from=$theme.children item=child}
                            {* Move to a new row *}
                            {if ($childrenInColumnCount == $theme.params.columns)}
                            {assign var="rel" value=""}
                        </tr>
                        <tr valign="top">
                            {assign var="childrenInColumnCount" value=0}
                            {/if}
                            {assign var=childrenInColumnCount value="`$childrenInColumnCount+1`"}
                            {if !isset($child.canContainChildren) && $child.entityType == 'GalleryPhotoItem'}
                              <td style="display:none;">
                                <a title="{$child.title}" href="{g->url arg1="view=core.DownloadItem" arg2="itemId=`$child.resizeId`"}"  rel="shadowbox[gallery];player=img" style="display:none;"> no image</a>
                              </td>
                            {elseif !isset($child.canContainChildren) }
                              <td style="display:none;">
                                <a href="{g->url arg1="view=core.ShowItem" arg2="itemId=`$child.id`"}" style="display:none;"> no image</a>
                              </td>
                            {else}
                            <td class="{if $child.canContainChildren}giAlbumCell gcBackground1{else}giItemCell{/if}" style="width: {$theme.columnWidthPct}%">
                                  ...
                                  {capture assign=linkUrl}{strip}
                                    ...
                                    {else}
                                      {if $child.entityType == 'GalleryPhotoItem'}
                                        {g->url arg1="view=core.DownloadItem" arg2="itemId=`$child.resizeId`"}
                                      {else}
                                        {g->url arg1="view=core.ShowItem" arg2="itemId=`$child.id`"}
                                      {/if}
                                    {/if}
                                ...
                                {/if}
                                {capture assign=rel}
                                  {if $child.entityType == 'GalleryPhotoItem'}shadowbox[gallery];player=img{/if}
                                {/capture}
                                <div>
                                    {if isset($theme.params.$frameType) && isset($child.thumbnail)}
                                      ...
                                      <a title="{$child.title}" href="{$linkUrl}" rel="{$rel}">
...
<script src="{g->theme url="js/shadowbox.js"}">
</script>
{literal}
<script>
    (function(){
        Shadowbox.init({
            overlayOpacity: 0.8
        }, Shadowbox.setup('#gsThumbMatrix a[rel="shadowbox[gallery]"]', {
            gallery: 'My Gallery',
            continuous: true,
            counterType: 'skip'
        }));
