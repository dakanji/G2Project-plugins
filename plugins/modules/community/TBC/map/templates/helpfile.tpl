...
var _HP_MapWidth = '{g->text text="This is the Width of the Map.<br/><br/>You have a choice between a size in <u>Pixels</u> or a size in <u>Percentage</u>.<br/><br/>Pixels will be an exact size, Percentage will be dependent on the window size and the things on the page that can reduce the size (sidebar, etc...)<br/><br/>Note: Percentage does not work very well with certain Themes." forJavascript=true}';
...
var _HP_F_Group = '{g->text text="This enables/disables the Group Management tab.<br/><br/>This allows you to create Groups which are items with GPS coordinates and a thumbnail picture. Each group can \"contain\" other groups, photos and albums.<br/><br/>When a group is selected on the map, it will show only the items contained into that group (it is like manual albums).<br/><br/>To see a group you have two choices: Either click on it on the default map and then click the thumbnail or else go directly to it via the address bar with g2_view=map.ShowMap&g2_group=G1 (for Group 1)." forJavascript=true}';
...
var _HP_KP_Name = '{g->text text="This is simply the name of the profile. It will appear in the list of keys when it is fully created." forJavascript=true}';
...
var _HP_OV_Position = '{g->text text="This is the position of the Overview Map.<br/><br/>By default it is on the bottom right-hand corner of the map.<br/><br/>Also, you can configure the window offset in pixels." forJavascript=true}';
...
var _HP_L_LegendPos = '{g->text text="This is the Legend Position.<br/><br/>If Left is selected it will be shown in the Sidebar (does not work with Gallery v2.2+).<br/><br/>If Top or Bottom is selected, it will be flat and will not have the expand feature explained below.<br/><br/>By default the Legend is hidden." forJavascript=true}';
...
var _HP_FI_BlockPos = '{g->text text="This is the Filter Block Position<br/><br/>If Left is selected it will be shown in the Sidebar (does not work with Gallery v2.2+).<br/><br/>If Top or Bottom is selected, it will be flat.<br/><br/>By default it is hidden." forJavascript=true}';

var _HP_FI_ManualFilter = '{g->text text="This allows you to add Manual filters.<br/><br/>Manual filters are a set of coordinates and a zoom level to center the map on a specific area; it does not change the Map data.<br/><br/>You can add, remove or edit filters; Add or Edit will show the same window." forJavascript=true}';
...
var _HP_M_Distance = '{g->text text="This is the distance (in pixels) at which items get regrouped. This can be changed so that different icon sizes would not overlap.<br><br>The default is 15 pixels." forJavascript=true}'
