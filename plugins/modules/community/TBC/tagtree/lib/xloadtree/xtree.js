	...
	if (root.rendered) {
		...
		while (foo.parentNode) {
			...
			if (i == foo.parentNode.childNodes.length - 1) {
				foo.parentNode._last = true; } else {
...
WebFXTreeAbstractNode.prototype.indent = function (lvl, del, last, level, nodesLeft) {
	...
	if (foo) {
		if ((foo._last) || ((del) && (last))) {
			foo.src = webFXTreeConfig.blankIcon; } else {
...
WebFXTree.prototype.getSelected = function () {
	if (webFXTreeHandler.selected) {
		return webFXTreeHandler.selected; } else {
		return null; }
}
...
WebFXTree.prototype.keydown = function (key) {
	if (key == 39) {
		if (!this.open) {
			this.expand(); } else if (this.childNodes.length) {
...
WebFXTreeItem.prototype.getLast = function () {
	if (this.childNodes[this.childNodes.length - 1].open) {
		return this.childNodes[this.childNodes.length - 1].getLast(); } else {
...
WebFXTreeItem.prototype.getNextSibling = function () {
	...
	if (++i == this.parentNode.childNodes.length) {
		return this.parentNode.getNextSibling(); } else {
...
WebFXTreeItem.prototype.getPreviousSibling = function (b) {
	...
	if (i == 0) {
		...
		if ((this.parentNode.childNodes[--i].open) || (b && this.parentNode.childNodes[i].folder)) {
			return this.parentNode.childNodes[i].getLast(); } else {
			return this.parentNode.childNodes[i]; }
		} }
