...
public class puzzle extends Applet implements Runnable, MouseListener
{
   ...
   public void init()
     {
        ...
        //
        totalpieces = integerFromString(getParameter("PUZ_NUM"));
        piecesx = integerFromString(getParameter("PUZ_NUMX"));
        piecesy = integerFromString(getParameter("PUZ_NUMY"));
        i = 0;
        if (totalpieces < 1) i++;
        if (piecesx < 1) i++;
        if (piecesy < 1) i++;
        ...
        r = getSize();
        PaintScreen = 0;
        PuzzleType = integerFromString(getParameter("PUZ_TYPE"));
        if (PuzzleType > 2 || PuzzleType < 0)
          PuzzleType = 0;

	DrawXMethod = integerFromString(getParameter("PUZ_XSTYLE"));
	if (getParameter("PUZ_XSTYLE") == "null" ||
	    getParameter("PUZ_XSTYLE") == null)
	  DrawXMethod = 0;

        //
        // If there is no status bar, remove all padding for it!
        //
        if (integerFromString(getParameter("PUZ_BAR")) == 0 &&
            getParameter("PUZ_BAR") != null &&
            getParameter("PUZ_BAR") != "null")
          {
             TOP_MARGIN = 0;
          }
   ...
   public void DisplayStatus(Graphics g) 
	...
	if (Bar_Positions == -1)
	  {
	     Bar_Positions = integerFromString(getParameter("PUZ_BAR"));
	     if (getParameter("PUZ_BAR") == "null" ||
		 getParameter("PUZ_BAR") == null)
	       Bar_Positions = 1357;
	  }
        ...
        if (NumberCorrect() == totalpieces)
          {
             ...
             if (getParameter("PUZ_PREFIXURL") != null && getParameter("PUZ_PREFIXURL") !=
                 "null")
               prefix = new String(getParameter("PUZ_PREFIXURL"));
             else
               prefix = new String("");
             
             if (getParameter("PUZ_POSTFIXURL") != null && getParameter("PUZ_POSTFIXURL") !=
                 "null")
               appended = new String(getParameter("PUZ_POSTFIXURL"));
             else
               appended = new String("");
             
             if (getParameter("PUZ_URL") == null || getParameter("PUZ_URL") == "null")
               return;
             
             try {
                NewUrl = new URL(getDocumentBase(), getParameter("PUZ_URL") + "?" + 
   ...
   public void run()
	...
	r = getSize();
        ...
        // Load the image
        //
        if (getParameter("PUZ_SRC") == null)
          Error("Image filename not defined.");
        // MIKE ROLLINS
	//WholePicture = getImage(getDocumentBase(), getParameter("PUZ_SRC"));
        try 
          {
            WholePicture = getImage(new URL(getParameter("PUZ_SRC")));
          } 
        catch (java.net.MalformedURLException urlError)
          {
            WholePicture = null;
            Error("Cannot obtain image from malformed src url!");
          }
	...
	//
	int ResizeOption = integerFromString(getParameter("PUZ_RESIZE"));
	if ((r.width != WholePicture.getWidth(this) ||
            ...
            ResizeOption != 0)
	  {
        ...
        //
        if (getParameter("PUZ_SOUNDSRC") != null && 
            getParameter("PUZ_SOUNDSRC") != "null")
          DoneSound = getAudioClip(getDocumentBase(), getParameter("PUZ_SOUNDSRC"));
        else
          DoneSound = null;
