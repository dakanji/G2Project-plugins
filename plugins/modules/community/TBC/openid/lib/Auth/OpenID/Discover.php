...
class Auth_OpenID_ServiceEndpoint {
	...
	public function fromHTML($uri, $html) {
		...
		foreach ($discovery_types as $triple) {
			...
			$urls = Auth_OpenID_legacy_discover(
				$html,
				$server_rel,
				$delegate_rel
			);
	...
	public function Auth_OpenID_findOPLocalIdentifier($service, $type_uris) {
	...
	public function filter_MatchesAnyOpenIDType(&$service) {
	...
	public function Auth_OpenID_getOPOrUserServices($openid_services) {
		...
		$openid_services = Auth_OpenID_arrangeByType(
			$openid_services,
	...
	public function Auth_OpenID_makeOpenIDEndpoints($uri, $yadis_services) {
		$s = array();
		if (!$yadis_services) {
			return $s;
		}
		...
		foreach ($yadis_services as $service) {
			...
			if ($type_uris
				...
				foreach ($uris as $service_uri) {
					...
					if ($openid_endpoint->parseService(
						$uri,
						$service_uri,
						$type_uris,
						$service
	...
	public function Auth_OpenID_discoverWithYadis(
		$uri,
		&$fetcher,
		...
		$openid_services = call_user_func_array(
			$endpoint_filter,
	...
	public function Auth_OpenID_discoverURI($uri, &$fetcher) {
		$uri = Auth_OpenID::normalizeUrl($uri);
		return Auth_OpenID_discoverWithYadis($uri, $fetcher);
	}

	public function Auth_OpenID_discoverWithoutYadis($uri, &$fetcher) {
